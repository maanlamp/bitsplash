...
- **There is no such thing as "paused". There are two pauses, and they are named apart.** A player pausing the game and a developer halting the ticker are different operations, and calling both "pause" is what let the game host and the editor's run host drift into two incompatible frames. The vocabulary, fixed here and implemented once in `engine/runtime/host.ts`:
  - **Gameplay pause** (`setGameplayPaused` / `gameplayPaused`) is the player's pause. The world stops simulating; **the UI keeps stepping and laying out**, because a pause menu that does not respond is not a pause menu. It is identical in every consumer — the shipped game and a run inside the editor — and it is the only pause the game layer knows about.
  - **Freeze** (`freeze` / `frozen` / `stepOnce`) is the editor's debugger. **No ticker advances at all** — not gameplay, not UI, not actions — and `stepOnce` advances exactly one frame. Rendering continues, so a frozen frame stays on screen and can be inspected. Freeze is editor tooling; the game never freezes.
    Both run through the one `Host.step`, so neither is a second implementation of a frame. Never add a boolean called `paused` to a host, a scene, or a world, and never widen either of these to mean the other.
- **Per-frame code allocates nothing.** Every render system, and every update system that runs each frame, must not allocate on its steady-state path — no object or array literals, no `map`/`filter`/`slice`/spread, no closures created per entity, no strings built per frame. Write into caller-owned buffers and reuse instance-owned scratch: `sampleInto`-style out-parameters instead of returned objects, parallel `px`/`py` number arrays instead of arrays of points, pooled typed arrays sized once and refilled, index loops instead of iterator chains, and hoisted module-level constants instead of literals rebuilt per call. This is not a micro-optimisation: a frame loop running at hundreds of hertz turns small per-entity allocations into tens of megabytes a second, and the resulting collections are seen as stutter, not as memory. Allocate at construction, on state change, or when a pool grows — never per frame.
