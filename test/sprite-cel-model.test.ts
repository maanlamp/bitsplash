...
/**
 * A 2-frame, 2-layer, 1-tag document on the real canvas-free
 * {@link SpriteEditCore} — the artifact the editor ships, no DOM shell.
 */
const twoFrameCore = (): SpriteEditCore => {
	...
	return SpriteEditCore.fromDescription(desc);
};

describe("cels model — active frame + transforms", () => {
	test("the active frame selects that frame's cels", () => {
		const core = twoFrameCore();
		expect(core.activeFrameIndex).toBe(0);
		expect(core.getCel("base", 0)!.data[(1 * 4 + 1) * 4 + 3]).toBe(
			255,
		);
		core.setActiveFrame(1);
		expect(core.snapshot().frameIndex).toBe(1);
		// The active cel is frame 1's base cel (pixel at 2,2), not frame 0's.
		expect(core.snapshot().data.data[(2 * 4 + 2) * 4 + 3]).toBe(255);
		expect(core.snapshot().data.data[(1 * 4 + 1) * 4 + 3]).toBe(0);
	});

	test("flip horizontal flips every frame's cels and is its own inverse", () => {
		const core = twoFrameCore();
		const f0 = core.getCel("base", 0)!.data.slice();
		const f1 = core.getCel("base", 1)!.data.slice();
		core.flipHorizontal();
		// pixel (1,1) → (2,1) on a width-4 image.
		expect(core.getCel("base", 0)!.data[(1 * 4 + 2) * 4 + 3]).toBe(
			255,
		);
		// frame 1's cel also moved.
		expect(core.getCel("base", 1)!.data[(2 * 4 + 1) * 4 + 3]).toBe(
			255,
		);
		core.flipHorizontal();
		expect(core.getCel("base", 0)!.data).toEqual(f0);
		expect(core.getCel("base", 1)!.data).toEqual(f1);
	});
	...
	test("rotate CW then CCW is the identity with dimensions restored", () => {
		...
		const core = SpriteEditCore.fromDescription(desc);
		const before = core.getCel("l", 0)!.data.slice();
		...
		core.rotateCcw();
		expect(core.width).toBe(3);
		expect(core.height).toBe(2);
		expect(core.getCel("l", 0)!.data).toEqual(before);
	});
});
...
describe("frame commands — real inverses", () => {
	test("addFrame inserts after active; undo removes it", async () => {
		...
		addFrame(core, history, 0);
		expect(core.frames.length).toBe(3);
		history.undo();
		await history.settle();
		expect(core.frames.length).toBe(2);
	});

	test("deleteFrame restores duration + per-layer cels + tags on undo", async () => {
		const core = twoFrameCore();
		const history = new History();
		const base1 = core.getCel("base", 1)!.data.slice();

		deleteFrame(core, history, 1);
		expect(core.frames.length).toBe(1);
		expect(core.getCel("base", 1)).toBeNull();
		// The tag range clamped into the shorter timeline.
		expect(core.tags[0]!.to).toBe(0);

		history.undo();
		await history.settle();
		expect(core.frames.length).toBe(2);
		expect(core.frames[1]!.duration).toBe(150);
		expect(core.getCel("base", 1)!.data).toEqual(base1);
		expect(core.tags[0]!.to).toBe(1);
	});
	...
	test("duplicateFrame copies cels; undo removes the copy", async () => {
		const core = twoFrameCore();
		const history = new History();
		duplicateFrame(core, history, 0);
		expect(core.frames.length).toBe(3);
		// The duplicate (new frame 1) carries frame 0's base + top cels.
		expect(core.getCel("base", 1)!.data[(1 * 4 + 1) * 4 + 3]).toBe(
			255,
		);
		expect(core.getCel("top", 1)!.data[0 + 3]).toBe(255);
		history.undo();
		await history.settle();
		expect(core.frames.length).toBe(2);
		// The original frame 1 (base pixel at 2,2) is back in place.
		expect(core.getCel("base", 1)!.data[(2 * 4 + 2) * 4 + 3]).toBe(
			255,
		);
	});
	...
	test("setFrameDuration undo/redo restores prior duration", async () => {
		const core = twoFrameCore();
		const history = new History();
		setFrameDuration(core, history, 0, 250);
		expect(core.frames[0]!.duration).toBe(250);
		history.undo();
		await history.settle();
		expect(core.frames[0]!.duration).toBe(100);
		history.redo();
		await history.settle();
		expect(core.frames[0]!.duration).toBe(250);
	});
...
describe("tag commands — real inverses", () => {
	test("create / rename / range / loop / delete undo cleanly", async () => {
		...
		createTag(core, history, {
			name: "run",
			from: 0,
			to: 1,
			loop: false,
		});
		expect(core.tags.map((t) => t.name)).toEqual(["idle", "run"]);

		renameTag(core, history, 1, "walk");
		expect(core.tags[1]!.name).toBe("walk");

		setTagRange(core, history, 1, 1, 1);
		expect([core.tags[1]!.from, core.tags[1]!.to]).toEqual([1, 1]);

		setTagLoop(core, history, 1, true);
		expect(core.tags[1]!.loop).toBe(true);

		deleteTag(core, history, 1);
		expect(core.tags.map((t) => t.name)).toEqual(["idle"]);
		...
		history.undo();
		await history.settle();
		expect(core.tags[1]!.loop).toBe(false);
		history.undo();
		await history.settle();
		expect([core.tags[1]!.from, core.tags[1]!.to]).toEqual([0, 1]);
		history.undo();
		await history.settle();
		expect(core.tags[1]!.name).toBe("run");
		history.undo();
		await history.settle();
		expect(core.tags.map((t) => t.name)).toEqual(["idle"]);
	});
...
describe("multi-frame writer → facade round-trip", () => {
	test("a 2-frame, 2-layer, 1-tag document serializes and reads back", () => {
		const core = twoFrameCore();
		const bytes = serializeBsprite(core.toSnapshot());
